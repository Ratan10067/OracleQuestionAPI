  ...
  fileFilter: (req, file, cb) => {
    ...
    if (folder === "profileimages" || folder === "questionimages") {
      ...
      if (
        ...
        return cb(
          new Error("Only images and PDFs are allowed for resumes"),
          false,
        );
      }
    } else if (folder === "testcases") {
      if (file.mimetype !== "application/json") {
        return cb(
          new Error("Only JSON files are allowed for testcases"),
          false,
        );
      }
    }
  ...
  try {
    ...
    const questions = files.map((file) => {
      const data = JSON.parse(
        fs.readFileSync(path.join(QUESTIONS_DIR, file), "utf8"),
      );
...
app.post("/questions", requireApiKey, (req, res) => {
  try {
    ...
    const id =
      questionData._id || require("crypto").randomBytes(12).toString("hex");
    questionData._id = id;
    ...
    res.status(201).json({
      ...
      data: {
        _id: id,
        questionUrl: `http://localhost:${PORT}/questions/${id}`,
      },
    });
  } catch (error) {
...
app.post("/files/:folder", requireApiKey, (req, res) => {
  console.log("File Uploaded", req.params.folder);
  fileUpload.single("file")(req, res, (err) => {
    ...
    console.log(
      `[UPLOAD] ${folder}/${filename} (${(req.file.size / 1024).toFixed(1)}KB)`,
    );
    res.status(201).json({
      success: true,
      data: {
